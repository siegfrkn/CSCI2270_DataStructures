...
// AND LINKED LIST BUT ISSUES WITH LINKED LIST REPEATS
...
void addWordArray(WordItem *& arrayUniqueWords, string textWord, int textCount, int arrayCount);
 ...
 * @param wordItemSLL - a pointer that points to a class of sllUniqueWords
...
int addNode(LinkedList *sllUniqueWords, string word, int sllCount){
	...
	// SingleNode *left = searchNode(sllUniqueWords, leftValue.word);
...
int main(int argc, char **argv){
	...
	int sllCount = 0;
	double sllTime = 0;
	// cout << sllUniqueWords -> head -> w.word << endl;
	...
	// Uncomment to print singly linked list for debugging
	...
	// cout << "\nNumber of items in linked list: " << sllCount << endl;
