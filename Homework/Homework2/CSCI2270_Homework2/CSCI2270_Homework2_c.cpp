...
ReturnResult readInFileArray(string fileMainString, int *arraySize, WordItem *&arrayUniqueWords, string arrayIgnoreWords[], int ignoreCount){
	...
	while (myMainFileStream.good()){
		...
		while ((position = singleLine.find(delimiter)) <= nposition){
			...
			// if it is a stop word, skip on to the next word
			// else{
				// break;
			// }
 ...
 * @return - none
*/
double BubbleSort(WordItem *&A, int length){
   time_t Tbegin, Tend;
   time(&Tbegin);
...
// 		WordItem index = A[i];
// 		int j=i;
// 		while (j>0 && A[j-1].count>index.count){
...
int main(int argc, char **argv){
	...
	// quickSort(arrayWords, 0, uniqueResults.value1);
	insertionSort(arrayWords, uniqueResults.value1);
	...
	cout << "\nNumber of ignored words: " << ignoreCount <<endl;
