 ...
 * CLASS MEMBER: Public
 * INPUTS: s of type S (right now, expected to be not a string)
 * PURPOSE: Takes a non-string and turns it into a string
 * OUTPUTS: true if it is a number, false otherwise
*/
template<class T, class S>
...
int main() {
	...
	// Expected template types for T, S respectively, works justs fine
	// for all number types (i.e. not passing string as S)
	// Library<int,string> library;

	// Expected template types for T, S respectively, does not work the
	// way I was hoping. I intend to solve this in my final project by
	// using template specialization but have not yet successfully
	// implemented this
	// Library<int,int> library;


	/*******************ADD EXPECTED STRUCT OBJECTS*******************/

	// COMMENT / UNCOMMENT THE FOLLOWING LINES - DOES WORK AS EXPECTED
	...
	// library.hashSum(bookToAdd2, hashSizePrime2);


	/*******************ADD UNEXPECTED STRUCT OBJECTS******************/
	
	// // DOES NOT WORK - NEED TO INSTANTIATE TEMPLATE SPECIALIZATION!!!!!
	// // Comment out the instantiation of the first Library object!
	// Library<double,string> library;
	// string bookToAdd = "Forrest Gump";
	// // cout << typeid(string).name() << endl;
	// int primeSize = sizeof(bookToAdd);
	// cout << "size of bookToAdd "<< bookToAdd << endl;
...
/***********************DEMONSTRATE FUNCTIONALITY**********************/
	...
	// UNCOMMENT TO SEE HOW INDIVIDUAL ADDED FUNCTIONS WORK

	// // DEMONSTRATE ISNUMBER FUNCTION - DOES NOT WORK THE WAY I INTENDED
	// // requires template S to be a string
	// Library<int,int> library;
	// int number = 894376;
	// library.isNumber(number);

	// // DEMONSTRATE FUNCTIONALITY OF TAKING A NUMBER TO A STRING - DOES WORK
	// // COMMENT OUT THE FIRST LIBRARY OBJECT
	// Library<int,int> library;
	// int number = 894376;
	// string output = library.intToString(number);
	// cout << "I know " << output << " is a string because I can index it, " << output << "[0]." << endl;
	// cout << "The first letter of " << output << " is " << output[0] << endl;
