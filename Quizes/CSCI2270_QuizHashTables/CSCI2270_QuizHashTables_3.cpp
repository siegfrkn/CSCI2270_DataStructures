...
template<class T, class S, class ptr>
void hashTable<T,S,ptr>::addBook (S title) {
	numBooks++;
	if (numBooks == 1) {
		S *bookShelfArray = new (nothrow) S[numBooks];
	}
	// else if (numBooks > bookShelfArray.size()) {
	else {
		S *tmp = new (nothrow) S[numBooks];
		for (int i=0; i<numBooks-1; i++) {
			// while (bookShelfArray[i] != "") {
				tmp[i] = bookShelfArray[i];
			// }
		}
		S *bookShelfArray = new (nothrow) S[2*numBooks];
	}
	// for (int j=0; j<numBooks; j++) {
			bookShelfArray[numBooks] = title;
			// break;
		// }
	// }
}
